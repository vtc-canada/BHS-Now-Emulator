...
public class CommunicationMaster : MonoBehaviour {
	...
	conveyerTurn[] conveyer_turn_run;
	...
	MergeConveyer[] conveyer_merge_run;
	...
	Control_station_button[] cs_button_output;//lighting
	...
	void Start () {
		...
		foreach (object o in obj)
		{
			...
			else if(g.GetComponent<conveyerForward>()!=null){
				if(emu_to_plc.ContainsKey(g.GetComponent<conveyerForward>().IO_name_disconnect))
				{
					conveyer_disconnect_count ++;
				}
			}
			else if(g.GetComponent<conveyerTurn>()!=null){
				if(emu_to_plc.ContainsKey(g.GetComponent<conveyerTurn>().IO_name_disconnect))
				{
					conveyer_turn_disconnect_count ++;
				}
			}
			else if(g.GetComponent<MergeConveyer>()!=null){
				if(emu_to_plc.ContainsKey(g.GetComponent<MergeConveyer>().IO_name_disconnect))
				{
			...
			}else if(g.GetComponent<Control_station_button>()!=null){
		...
		conveyer_disconnect = new conveyerForward[conveyer_disconnect_count];
		...
		conveyer_turn_disconnect = new conveyerTurn[conveyer_turn_disconnect_count];
		...
		conveyer_merge_disconnect = new MergeConveyer[conveyer_merge_disconnect_count];
		...
		foreach (object o in obj)
		{
			...
			else if(g.GetComponent<conveyerForward>()!=null)
			{
				if(emu_to_plc.ContainsKey(g.GetComponent<conveyerForward>().IO_name_disconnect))
				{
					conveyer_disconnect[conveyer_disconnect_count] = g.GetComponent<conveyerForward>();
			...
			else if(g.GetComponent<conveyerTurn>()!=null)
			{
				if(emu_to_plc.ContainsKey(g.GetComponent<conveyerTurn>().IO_name_disconnect))
				{
					conveyer_turn_disconnect[conveyer_turn_disconnect_count] = g.GetComponent<conveyerTurn>();
			...
			else if(g.GetComponent<MergeConveyer>()!=null)
			{
				if(emu_to_plc.ContainsKey(g.GetComponent<MergeConveyer>().IO_name_disconnect))
				{
					conveyer_merge_disconnect[conveyer_merge_disconnect_count] = g.GetComponent<MergeConveyer>();
		...
		{
			GameObject g = (GameObject) o;
			if(g.GetComponent<conveyerForward>()!=null)
			{
				if(plc_to_emu.ContainsKey(g.GetComponent<conveyerForward>().IO_name_motor_run))
				{
					conveyercount++;
				}
			}
			...
			{
				if(plc_to_emu.ContainsKey(g.GetComponent<conveyerTurn>().IO_name_motor_run))
				{
			...
			{
				if(plc_to_emu.ContainsKey(g.GetComponent<MergeConveyer>().IO_name_motor_run))
				{
		...
		conveyer = new conveyerForward[conveyercount];		
		...
		conveyer_turn_run = new conveyerTurn[conveyer_turn_run_count];		
		...
		conveyer_merge_run = new MergeConveyer[conveyer_merge_run_count];		
		...
		cs_button_output = new Control_station_button[cs_button_output_count];
		...
		foreach (object o in obj)
		{
			...
			if(g.GetComponent<conveyerForward>()!=null)
			{
				if(plc_to_emu.ContainsKey(g.GetComponent<conveyerForward>().IO_name_motor_run))
				{
					conveyer[conveyercount] = g.GetComponent<conveyerForward>();
			...
			else if(g.GetComponent<conveyerTurn>()!=null)
			{
				if(plc_to_emu.ContainsKey(g.GetComponent<conveyerTurn>().IO_name_motor_run))
				{
					conveyer_turn_run[conveyer_turn_run_count] = g.GetComponent<conveyerTurn>();
			...
			else if(g.GetComponent<MergeConveyer>()!=null)
			{
				if(plc_to_emu.ContainsKey(g.GetComponent<MergeConveyer>().IO_name_motor_run))
				{
					conveyer_merge_run[conveyer_merge_run_count] = g.GetComponent<MergeConveyer>();
			...
			}else if(g.GetComponent<Control_station_button>()!=null)
			{
				if(plc_to_emu.ContainsKey(g.GetComponent<Control_station_button>().IO_output))
				{
					cs_button_output[cs_button_output_count] = g.GetComponent<Control_station_button>();
